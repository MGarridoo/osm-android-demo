package com.example.osmdemo.features.trips_details.presentation.adapters
...
class TripsDetailsAdapter(
    ...
    private val onClick: (leg: Leg) -> Unit = {},
            ...
            when(leg.type) {
                ...
                "JNY" -> {
                    ...
                    with(binding.busDetailItem.recyclerDetails) {
                        layoutManager = LinearLayoutManager(context)
                        adapter = leg.stops?.let {
                            ItemStopAdapter(it.stop)
                        }
                        visibility = if (isExpanded) View.VISIBLE else View.GONE
                    }

                    binding.busDetailItem.tvName.setOnClickListener { onClick(leg) }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): TripsDetailsAdapter.JourneyViewHolder {
        val binding = ItemJourneyBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        return JourneyViewHolder(binding)
    }

    override fun onBindViewHolder(holder: TripsDetailsAdapter.JourneyViewHolder, position: Int) {
